...
/**
 * Tests for Fix 6: Tracking endpoint privacy hardening.
 */
class TrackingEndpointHardeningTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_tracking_without_email_hash_returns_limited_data(): void
    {
        ...
        $response->assertOk();
        $response->assertJsonPath('data.tracking_code', $courseRequest->public_tracking_code);
    ...
    public function test_tracking_with_valid_email_hash_returns_full_data(): void
    {
        ...
        $response = $this->getJson('/api/v1/course-requests/ABCD1234EFGH5678?email_hash='.hash('sha256', 'test@example.com'));

        $response->assertOk();
        $response->assertJsonPath('data.payment_proof_status', null);
    }
    ...
    public function test_tracking_with_invalid_email_hash_returns_404(): void
    {
        ...
        $response = $this->getJson('/api/v1/course-requests/ABCD1234EFGH5678?email_hash='.hash('sha256', 'wrong@example.com'));
